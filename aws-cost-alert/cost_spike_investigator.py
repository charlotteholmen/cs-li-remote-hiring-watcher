...
class CostSpikeInvestigator:
                ...
                for event_source in service_event_mapping[top_service]:
                    try:
                        ...
                        for event in response['Events']:
                            ...
                            user_activity[event.get('Username', 'Unknown')] += 1
    ...
    def run_investigation(self):
        ...
        for spike in spikes:
            ...
            next_date = (datetime.fromisoformat(spike_date) + timedelta(days=1)).date().isoformat()
            cloudtrail_analysis = self.investigate_cloudtrail_events(spike_date, next_date, services)
            ...
            if any(s['service'] == 'AWS Cost Explorer' for s in services):
                cost_explorer_events = self.check_cost_explorer_usage(spike_date, next_date)
                if cost_explorer_events:
                    print(f"\n💰 Cost Explorer Events Found:")
                    for event in cost_explorer_events[:10]:
                        print(f"  🕐 {event['time']}: {event['event']} by {event['user']}")
